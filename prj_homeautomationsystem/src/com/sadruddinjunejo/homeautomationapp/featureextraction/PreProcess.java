...
public class PreProcess {
      ...
      private void doFraming() {
    	  	...
    	  	Log.d("After end point bla", "" + afterEndPtDetection.length);
    	  	 noOfFrames = NUMBER_OF_FRAMES;
              ...
              Log.d("Criteria", "Number of frames:" + noOfFrames + "| Samples per frame:" + samplePerFrame);
      ...
      private void doWindowing() {
              ...
              for (int i = 1; i <= samplePerFrame; i++) {
                      hammingWindow[i] = (float) (0.54f - 0.46f * (Math.cos(2 * Math.PI * i / samplePerFrame)));
              }
              // do windowing
              for (int i = 0; i < noOfFrames; i++) {
                      for (int j = 0; j < samplePerFrame; j++) {
